...
async function extractAndSaveMemories(userId: number, userMessage: string): Promise<void> {
  try {
    ...
    const all = await db
      ...
      .orderBy(desc(userMemories.createdAt));
    if (all.length > 30) {
      for (const id of all.slice(30).map((r) => r.id)) {
  ...
  if (getAiProvider() === "local") {
    ...
    try {
      ...
      const localSystem = [
        ...
        "Image generation is not available in local mode.",
      ].filter(Boolean).join("\n\n");
      ...
      if (localTools.length === 0) {
        ...
        for (let turn = 0; turn < 3; turn++) {
          ...
          if (userId && (await isGithubReady(userId))) {
            ...
            result = { error: "No GitHub repository is connected/selected. Connect GitHub and pick a repo in Settings." };
          }
