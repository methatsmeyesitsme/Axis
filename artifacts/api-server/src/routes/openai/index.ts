...
router.post("/conversations/:id/messages", async (req, res) => {
  ...
  try {
    ...
    if (wantsGithubTool) {
      // ALWAYS pull for real — public API when owner/repo is in the message, else connected repo.
      // Never ask the local model to invent tool JSON.
      ...
      // Always attach the user's PAT for private repos like methatsmeyesitsme/Axis.
      ...
      if (readme?.path && !listed.error) {
        ...
        if (parsed) {
          read = await readGithubFilePublic(parsed.owner, parsed.repo, readme.path, "main", userToken);
        } else if (userId) {
      ...
      // Hard guarantee: never save/stream a tool name
      if (isToolLeak(reply)) {
