...
export const forgeToolDeclarations: FunctionDeclaration[] = [
  ...
  },
  {
    ...
    description:
      "Pull / clone / load the user's connected GitHub repository into this Forge app for preview. " +
      "Call this whenever the user asks to pull, pull again, pull the repo, pull from GitHub, load my repo, " +
      "or import their connected repository — including short requests like just 'pull'.",
    parametersJsonSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Load a Vite/React package dist into this Forge app (prefers existing dist for speed). package: axis-preview, tidy-toters, etc.",
    parametersJsonSchema: {
      type: "object",
      properties: {
...
<h1>This package needs a production dist</h1>
<p>Promoted <code>${fromPath}</code> — Vite/React source shell.</p>
<p>Build once in the shell: <code>cd artifacts/axis-preview && pnpm run build</code> — then pull again (a few seconds).</p>
<div class="box">
<p style="margin:0;color:#e2e8f0"><strong>What works</strong></p>
<ul>
<li>Shell build once, then pull / build in Forge (loads dist in seconds)</li>
...
async function importGithubIntoForge(
  ...
  const needsBuild =
    ...
    /needs a full build|needs a production dist|Vite\/React/i.test(rootFile.content);

  let built: { package?: string; files?: number; buildMs?: number } | null = null;
  let buildError: string | null = null;
  // Dist-only load (no multi-minute Vite in the chat request). Under ~15s.
  if (needsBuild) {
  ...
  return {
    output: {
      ...
      buildError,
      hint: built
        ? `Loaded ${built.package} from dist (${built.files} files, ${built.buildMs ?? "?"}ms). Press Run to preview.`
        : buildError
          ? `Pulled files. ${buildError.slice(0, 500)}`
          : hasIndex
            ? promoted
              ? `Promoted ${promoted} → index.html. Press Run.`
              : "Preview is ready — press Run."
            : "Imported files, but no HTML entry.",
    },
  };
}
  ...
  try {
    switch (name) {
      ...
      case "build_workspace_app": {
        ...
        if (!result.ok) {
          result = await buildWorkspacePackage(appId, pkg, { forceCompile: true });
        }
        ...
        return {
          output: {
            ...
            hint: result.reusedDist
              ? `Loaded ${result.package} from dist (${result.files} files, ${result.buildMs}ms). Press Run.`
              : `Built ${result.package} (${result.files} files, ${result.buildMs}ms). Press Run to preview.`,
          },
        };
      }
      case "run_preview": {
